...
def preprocess_mp3(mp3_fpath: Path, output_fpath: Path) -> None:
    """Preprocess an mp3 file and save the magnitudes to a CSV file."""
    ...
    times = (
        librosa.frames_to_time(np.arange(pitches.shape[1]), sr=sr) * 1000
    )  # Convert to ms
